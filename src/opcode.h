...
typedef enum opcode_enum {
    ...
    OPCODE_JUMP_FORWARD          = 113,  // Followed by 4 bytes (aka jump offset)
    OPCODE_ABSOLUTE_JUMP         = 114,  // Followed by 4 bytes (aka jump offset)
    OPCODE_POPTOP                = 115,  // No following bytes
    OPCODE_NOP                   = 116,  // No following bytes
    OPCODE_MAKE_FUNCTION         = 117,  // Followed by 4 bytes (aka parameter count) + 8 bytes (aka bytecode size)
    OPCODE_MAKE_ASYNC_FUNCTION   = 118,  // Followed by 4 bytes (aka parameter count) + 8 bytes (aka bytecode size)
    OPCODE_SETUP_BLOCK           = 119,  // Followed by 8 bytes (aka bytecode size)
    OPCODE_RETURN                = 120,  // No following bytes
    OPCODE_COMPLETE_BLOCK        = 121,  // No following bytes
    OPCODE_DUPTOP                = 122,  // No following bytes
    OPCODE_ROT2                  = 123,  // No following bytes
} opcode_t;
